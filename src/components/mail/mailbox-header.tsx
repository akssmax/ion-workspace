...
export function MailboxHeader({ mailboxId, query, page, onPageChange, sort, onSortChange, quickFilters, onQuickFiltersChange, sent }: { mailboxId: string | null; query: string; page: number; onPageChange: (page: number) => void; sort: MailSort; onSortChange: (sort: MailSort) => void; quickFilters: MailQuickFilter[]; onQuickFiltersChange: (filters: MailQuickFilter[]) => void; sent: boolean }) {
  ...
  return <div className="shrink-0"><header aria-label="Mailbox header" className="flex h-11 min-w-0 shrink-0 items-center gap-1 border-b px-2">
    ...
    {hasSelection ? <>
      ...
      <Tooltip><TooltipTrigger render={<Button variant="ghost" size="icon-touch" onClick={clearSelection} aria-label="Clear selection" />}><X className="size-4" /></TooltipTrigger><TooltipContent>Clear selection</TooltipContent></Tooltip>
    </> : <>
      <MailSortMenu sort={sort} onSortChange={onSortChange} sent={sent} iconOnly />
      <Tooltip><TooltipTrigger render={<Button variant={controlsOpen || quickFilters.length > 0 ? "secondary" : "ghost"} size="icon-touch" aria-label={`Filters${quickFilters.length ? `, ${quickFilters.length} active` : ""}`} aria-pressed={controlsOpen} onClick={() => setControlsOpen((open) => !open)} />}><Filter className="size-4" />{quickFilters.length > 0 ? <span className="ms-0.5 text-[10px] tabular-nums">{quickFilters.length}</span> : null}</TooltipTrigger><TooltipContent>Filters</TooltipContent></Tooltip>
    ...
    <div className="ms-auto flex shrink-0 items-center gap-0.5">
      ...
      <Tooltip><TooltipTrigger render={<Button variant="ghost" size="icon-touch" aria-label="Previous page" disabled={page === 0 || results.isPending} onClick={() => onPageChange(page - 1)} />}><ChevronLeft className="size-4 rtl:rotate-180" /></TooltipTrigger><TooltipContent>Previous page</TooltipContent></Tooltip>
      <Tooltip><TooltipTrigger render={<Button variant="ghost" size="icon-touch" aria-label="Next page" disabled={!hasNext || results.isPending || results.isError} onClick={() => onPageChange(page + 1)} />}><ChevronRight className="size-4 rtl:rotate-180" /></TooltipTrigger><TooltipContent>Next page</TooltipContent></Tooltip>
      {hasSelection ? null : <Tooltip><TooltipTrigger render={<Button variant="ghost" size="icon-touch" aria-label="Refresh" onClick={() => void queryClient.invalidateQueries({ queryKey: ["acc", "emails"] })} />}><RefreshCw className="size-4" /></TooltipTrigger><TooltipContent>Refresh</TooltipContent></Tooltip>}
    </div>
